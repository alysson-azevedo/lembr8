...
## 2026-08-14 — Dispatcher autônomo via Orca automations (time LB)
**Contexto:** o `docs/workflow.md` já prevê um dispatcher em loop (varre issues do time LB, despacha o agente da label de responsável), mas sua implementação estava adiada como "issue própria". O humano quer processamento automático das tarefas abertas **uma por vez** (paralelismo esgota limite de token), mantendo o workflow atual e o gate de review humano.
**Decisão (humano):** implementar o dispatcher como **automação agendada do Orca** (`orca automations`, trigger cron ~7 min, provider `claude`, sessão fresca por execução). Cada execução faz **um ciclo**: varre issues LB acionáveis, escolhe **uma** (prioridade: review humana pendente > trabalho em curso > Backlog sem dono), despacha **um** worker (gateway Ollama `ollama launch claude --model glm-5.2:cloud`), espera `worker_done` e avança o state. Issue em `👀 Preview Review` aguarda comentário humano; aprovação → deploy, reprovação → rework. `--precheck` com lockfile impede execuções sobrepostas. Decisões de arquitetura/stack/custo/credencial continuam escaladas ao humano (não auto-decididas). Lógica versionada em `docs/agents/dispatcher.md`.
**Consequências:** processamento sequencial e durável (independente de sessão do coordinator), dentro do limite de token por ciclo; autonomia total do Backlog até o review, alinhada ao `CLAUDE.md` (fora de pontos críticos, agentes têm autonomia). Limitação: automação roda enquanto o Orca está ativo no host; não é 24/7 server-side. Estado vive no Linear (sem arquivo de estado próprio).
