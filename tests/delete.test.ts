...
function localList(id: string, nome: string, updatedAt = T0): LocalList {
  return { id, nome, pinned: false, archivedAt: null, createdAt: T0, updatedAt };
}
function localItem(
  id: string,
  listId: string,
...
function cloudList(id: string, nome: string, updatedAt = T0): ListRecord {
  return { id, nome, pinned: false, archived_at: null, created_at: T0, updated_at: updatedAt };
}
function cloudItem(
  id: string,
  listId: string,
...
describe("createLocalFirstRepository — deleteItem/deleteLista (LB-8)", () => {
  ...
  it("cache v4 persiste deletedIds e recarrega preservando", () => {
    ...
    expect(JSON.parse(storageA.getItem("lembr8.data")!).version).toBe(6);
  ...
  it("migra cache v3 legado para v4 adicionando deletedIds", () => {
    ...
    // a migração v3→v4→v5→v6 é lazy em memória; uma mutação a persiste no novo formato.
